...
# Каноническое имя архитектуры: roo_tflite
# Добавляем алиасы на случай отличий в именовании
MODEL_TYPES = {
    "roo_tflite",
    "roo-tflite",
    "roo",          # если вдруг кто-то так сохранил
    "rootflite",    # встречается в некоторых пайплайнах
}
...
def _canon_model_type(s: str) -> str:
    """Нормализует строку model_type из имени файла/конфига к каноничному виду."""
    s = (s or "").strip().lower()
    s = s.replace("-", "_")
    # канонизация алиасов
    if s in ("roo", "rootflite", "roo_tflite", "roo_tflite_model"):
        return "roo_tflite"
    if s == "roo_tflite":
        return "roo_tflite"
    return s
...
def list_models(selected_models_dir: str) -> List[str]:
    ...
    for p in paths:
        try:
            ...
            if _canon_model_type(info["model_type"]) == "roo_tflite":
...
def can_handle_model(model_path: str) -> bool:
    try:
        ...
        return _canon_model_type(info["model_type"]) == "roo_tflite"
    except Exception:
        return False
