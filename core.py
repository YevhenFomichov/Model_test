...
ARCH_GROUPS = {
    "basic": {"basic"},
    "cnn": {"cnn_lstm", "cnn2d_resnet", "crnn"},
    "dual": {"dual_branch_time"},
    "roo": {"roo_tflite"},
    "tcn": {"tcn", "tcn_time"},
...
def load_audio_mono_from_bytes(audio_bytes: bytes, suffix: str, sr: int) -> np.ndarray:
    with tempfile.NamedTemporaryFile(delete=True, suffix=suffix) as tmp:
        ...
        tmp.flush()
        return load_audio_mono_from_path(tmp.name, sr)
...
def extract_logmel(audio: np.ndarray, sr: int, cfg: dict) -> np.ndarray:
    if not HAS_LIBROSA:
        raise RuntimeError("librosa required for logmel")
...
def extract_pcen_mel(audio: np.ndarray, sr: int, cfg: dict) -> np.ndarray:
    if not HAS_LIBROSA:
        raise RuntimeError("librosa required for pcen_mel")
    n_mels = int(cfg.get("n_mels", 64))
    n_fft = int(cfg.get("n_fft", 512))
    hop_length = int(cfg.get("hop_length", 256))
    mel = librosa.feature.melspectrogram(y=audio, sr=sr, n_mels=n_mels,
...
def extract_spectrogram(audio: np.ndarray, sr: int, cfg: dict) -> np.ndarray:
    if not HAS_LIBROSA:
        raise RuntimeError("librosa required for spectrogram")
...
def extract_mfcc(audio: np.ndarray, sr: int, cfg: dict) -> np.ndarray:
    if not HAS_LIBROSA:
        raise RuntimeError("librosa required for mfcc")
...
def extract_mfcc_deltas(audio: np.ndarray, sr: int, cfg: dict) -> np.ndarray:
    if not HAS_LIBROSA:
        raise RuntimeError("librosa required for mfcc_deltas")
    n_mfcc = int(cfg.get("num_mfcc", 13))
    n_fft = int(cfg.get("stft_n_fft", 512))
    hop_length = int(cfg.get("stft_hop_length", 256))
...
def parse_model_filename(model_path: str) -> Dict[str, Optional[str]]:
    ...
    return {"model_type": model_type, "feature_type": feature_type, "transformation_method": transform, "loss_type": loss}
...
def infer_arch_group(model_type: str) -> str:
    mt = str(model_type).lower()
    for group, mts in ARCH_GROUPS.items():
        if mt in mts:
            return group
    raise ValueError(f"Unknown model_type='{mt}' for arch grouping")

# =========================================================
# Keras artifact loading (.keras zip vs HDF5 renamed)
# =========================================================
...
def load_keras_model_any(model_path: str):
    import tensorflow as tf
    kind, diag = keras_artifact_kind(model_path)
    if kind == "lfs_pointer":
        raise RuntimeError(f"Git LFS pointer detected: {model_path}")
    if kind in ("missing", "unknown"):
        raise FileNotFoundError(f"Not a loadable model: kind={kind}, diag={diag}")
...
def pad_or_trim_2d(X: np.ndarray, target_T: int, target_D: int) -> np.ndarray:
    ...
    if X.ndim != 2:
        raise ValueError(f"Expected 2D, got {X.ndim}D")
    T, D = X.shape
    ...
    if T != target_T:
        if T > target_T:
            ...
            X = np.concatenate([X, np.zeros((target_T - T, target_D), dtype=np.float32)], axis=0)
